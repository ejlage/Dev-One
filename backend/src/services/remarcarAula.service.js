// T16 – Remarcar Aula
// ST62: Atualiza pedidodeaula.data, horainicio, salaidsala + estado → PENDENTE
// ST63: Trigger notificação ao encarregadoeducacao


import prisma from "../config/prisma.js";
import { existeConflitoSala, existeConflitoProf, timeParaMinutos } from "../utils/aulas.helpers.js";
...
export async function remarcarAula(idaula, data, horainicio, salaidsala, motivo) {
  ...
  const conflitoSala = await existeConflitoSala(salaidsalaFinal, new Date(data), inicioMin, fimMin, pedido.idpedidoaula);
  if (conflitoSala)
  ...
  const horainicioBD = new Date(`1970-01-01T${String(ih).padStart(2, "0")}:${String(im).padStart(2, "0")}:00`);
  ...
  await prisma.aula.update({ where: { idaula }, data: { salaidsala: salaidsalaFinal } });
  ...
  await prisma.aula.update({ where: { idaula }, data: { estadoaulaidestadoaula: estadoPendente.idestadoaula } });
  ...
  await notificarRemarcacao({ email: utilizador.email, nome: utilizador.nome, idaula, data, horainicio, motivo });
